...
TTEN_IUCN_STATUS <- read.csv2("TTEN_IUCN_STATUS.csv")
TTEN_IUCN_STATUS_BEES <- TTEN_IUCN_STATUS %>%
  filter(FAMILY %in% c("Andrenidae", "Apidae", "Colletidae", "Halictidae", "Megachilidae", "Melittidae")) %>%
  mutate(DATASET_TITLE = "Abeilles")
write.csv2(TTEN_IUCN_STATUS_BEES, "TTEN_IUCN_STATUS.csv")
